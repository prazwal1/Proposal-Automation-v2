...
- slug: `container-storage`  |  module: `container-storage-module`
...
- **`GiB`** (number)
  - depends on: `orchestrationCostsUnits`
  - disappears when: `orchestrationCostsUnits` = *TiB*

- **`orchestrationCostsUnits`** (select)
  - depends on: `orchestrationCostsUnits`
  - disappears when: `orchestrationCostsUnits` = *TiB*
  - options: GiB, TiB

- **`Type`** (select)
  - options: Azure Disks, Ephemeral Disk, Azure Elastic SAN

- **`Tier`** (select)
  - depends on: `type`
  - disappears when: `type` = *Ephemeral Disk*, `type` = *Azure Elastic SAN*
  - options: Standard HDD, Standard SSD, Premium SSD, Premium SSD V2, Ultra SSD

- **`Disk size`** (select)
  - depends on: `type`, `tier`
  - disappears when: `type` = *Ephemeral Disk*, `type` = *Azure Elastic SAN*, `tier` = *Premium SSD V2*
  ...
  - when `tier` = *Standard SSD*: E1: 4 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E2: 8 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E3: 16 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E4: 32 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E6: 64 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E10: 128 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E15: 256 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E20: 512 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E30: 1,024 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, E40: 2,048 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, ...
  - when `tier` = *Premium SSD*: P1: 4 GiB, 120 Provisioned IOPS, 25 Provisioned MB/s, P2: 8 GiB, 120 Provisioned IOPS, 25 Provisioned MB/s, P3: 16 GiB, 120 Provisioned IOPS, 25 Provisioned MB/s, P4: 32 GiB, 120 Provisioned IOPS, 25 Provisioned MB/s, P6: 64 GiB, 240 Provisioned IOPS, 50 Provisioned MB/s, P10: 128 GiB, 500 Provisioned IOPS, 100 Provisioned MB/s, P15: 256 GiB, 1,100 Provisioned IOPS, 125 Provisioned MB/s, P20: 512 GiB, 2,300 Provisioned IOPS, 150 Provisioned MB/s, P30: 1,024 GiB, 5,000 Provisioned IOPS, 200 Provisioned MB/s, P40: 2,048 GiB, 7,500 Provisioned IOPS, 250 Provisioned MB/s, ...
...
- **`Transaction units (10,000 transactions)`** (number)
  - depends on: `type`, `tier`
  - disappears when: `type` = *Ephemeral Disk*, `type` = *Azure Elastic SAN*, `tier` = *Premium SSD*, `tier` = *Premium SSD V2*, `tier` = *Ultra SSD*

- **`TiB`** (number)
  - depends on: `orchestrationCostsUnits`
  - only exists when: `orchestrationCostsUnits` = *TiB*

- **`TiB (orchestrationCostsUnits)`** (select)
  - depends on: `orchestrationCostsUnits`
  - only exists when: `orchestrationCostsUnits` = *TiB*
  - options: GiB, TiB
...
- **`Redundancy`** (select)
  - depends on: `type`
  - only exists when: `type` = *Azure Elastic SAN*
  - options: LRS, ZRS
...
```json
{
  "product": "Container Storage",
  "name": "my-container-storage",
  "fields": {
    "Region": "Central US",
    "GiB": 1,
    "orchestrationCostsUnits": "GiB",
    "Type": "Azure Disks",
    "Tier": "Standard HDD"
  }
}
